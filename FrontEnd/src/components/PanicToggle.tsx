...
export const PanicToggle: React.FC = () => {
  ...
  useEffect(() => {
    if (!activeSession || !activeSession.deadline) {
      setTimeLeft('');
      return;
    }
    ...
    const updateCountdown = () => {
      try {
        const now = new Date();
        const examDate = parseISO(activeSession.deadline);
        const hours = differenceInHours(examDate, now);
        if (hours > 24) {
          const days = Math.round(hours / 24);
          setTimeLeft(`🚨 ${days}d left (${activeSession.urgencyLevel.toUpperCase()})`);
        } else if (hours > 0) {
          setTimeLeft(`🚨 ${hours}h left (${activeSession.urgencyLevel.toUpperCase()})`);
        } else {
          setTimeLeft(`🚨 Exam now! (${activeSession.urgencyLevel.toUpperCase()})`);
        }
      } catch (err) {
        setTimeLeft('🚨 Panic Active');
      }
    };
  ...
  const handleActivate = async () => {
    try {
      ...
      const session = await panicApi.setPanicSession(targetHours, 'hours');
      setPanicSession(session);
      setUrgency((session.urgencyLevel as any) || 'high');
  ...
  const handleDeactivate = async () => {
    try {
      if (activeSession) {
        await panicApi.endPanicSession(activeSession.id);
      }
  ...
  const getUrgencyColor = () => {
    const tier = activeSession?.urgencyLevel || urgency;
    switch (tier) {
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.buttonRow}>
              ...
              <TouchableOpacity style={styles.activateButton} onPress={handleActivate}>
                <Text style={styles.activateText}>Set Panic</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
